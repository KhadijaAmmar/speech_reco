...
def wav2mfcc(file_path, max_len=22):
    ...
    #sr=44.1khz
    #S = librosa.feature.melspectrogram(wave.astype(float), sr=sr, n_mels=128)
    wave = wave[3000:14000]

# Convert to log scale (dB). We'll use the peak power (max) as reference.
    #mfcc= librosa.power_to_db(S, ref=np.max)
    mfcc1 = librosa.feature.mfcc(wave,sr, n_mfcc=40)  #sampling rate / default hop value 512
    mean = np.mean(mfcc1, axis=0)
    std = np.std(mfcc1, axis=0)
    mfcc = (mfcc1 - mean) / std
...
def save_data_to_array(path, max_len=22):
